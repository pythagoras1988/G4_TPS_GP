...
class ConvertDicomData:
	...
	def _convertData(self):
		...
		for k in range(len(fname)):
			...
			checkBehaviorIndex = self._saveDicomSpecs(ds,fname[k][0:-3]+'g4dcm')
			np.savetxt(new_fname,ds.pixel_array)
			if k == 0:
				FLAG_BEHAVIOR = checkBehaviorIndex
			else:
				if (FLAG_BEHAVIOR!=checkBehaviorIndex):
					raise Exception("Specs File are not well behaved. Please check....")
				FLAG_BEHAVIOR = checkBehaviorIndex

	def _saveDicomSpecs(self,ds,fname):
		checkBehaviorIndex = 0
		with open(self.fname_baseDir+'_specsFile.txt','a') as writeFile:
			writeFile.write("%s \t" %fname)
			writeFile.write("%f \t" %ds[0x18,0x50].value)
			checkBehaviorIndex += ds[0x18,0x50].value
			writeFile.write("%d \t" %ds[0x28,0x10].value)
			checkBehaviorIndex += ds[0x28,0x10].value
			writeFile.write("%d \t" %ds[0x28,0x11].value)
			checkBehaviorIndex += ds[0x28,0x11].value
			writeFile.write("%f \t" %ds[0x28,0x30].value[0])
			checkBehaviorIndex += ds[0x28,0x30].value[0]
			writeFile.write("%f \t" %ds[0x28,0x30].value[1])
			try:
				writeFile.write("%f \t" %ds[0x20,0x32].value[0])
				checkBehaviorIndex += ds[0x20,0x32].value[0]
				...
				checkBehaviorIndex += sum(ds[0x20,0x37].value[0:2])
			except:
				writeFile.write("-1 \n")
				raise ValueError("Invalid values in Dicom data...")

			return checkBehaviorIndex
